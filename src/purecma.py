...
from plotting import draw_contour, split_X
...
"""
    ...
    while not es.stop():
        ...
        fit = objective_fct(X, *args)  # evaluate candidates
...
class CMAES(OOOptimizer):  # could also inherit from object
    ...
    def tell(self, arx, fitvals):
        ...
        # draw_contour(a,arx,(-100,100),(-100,100),par.mu)
    ...
    def stop(self):
        ...
        if len(self.fitvals) > 1 \
                and self.fitvals[-1] - self.fitvals[0] < 1e-8:
            res['tolfun'] = 1e-8
...
"""
    ...
    def plot(self, title=None, fig_number=322, series_name:str=None, override_fmin=None):
        ...
        fmin =  fmin if not override_fmin else override_fmin
        semilogy(dat['iter'], [f - fmin if f - fmin > 1e-19 else None
                               for f in dat['fit']], linewidth=1, label=series_name)
